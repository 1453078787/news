function DataTable(options) {
    var defaults = {
        ...
        handleResult: function (response) {
            if(opts.isBaseResponse) {
                if(response.body && response.body.result) {
                    return {
                        'rows': response.body.result,
                        'total': response.body.pagination.total,
                        'pageCount': response.body.pagination.totalPage,
                        'page': response.body.pagination.page,
                        'pageSize': response.body.pagination.pagesize
                    }
                } else {
                    return {
                        'rows': response.body
    ...
    this.getRowHtml = function(row,index){
        ...
        for ( var j = 0; j < opts.columns.length; j++) {
            ...
            if (col.checkbox) {
                if(col.format && !col.format(val, row, index)){
                    bodyHtml += '<td></td>';
                } else {
        ...
        $.ajax({
            ...
            success : function(page) {
                if(page){
                    ...
                    if (typeof target.opts.handleResult === "function"){
                        target.page = target.opts.handleResult(page);
                    } else {
                        throw "handleResult is not function!"
                    }
